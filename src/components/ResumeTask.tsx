...
export function ResumeTask({
  ...
  if (loading) {
    return <Box flexDirection="column" padding={1}>
        <Box flexDirection="row">
          <Spinner />
          <Text bold>Loading CatchCat Code sessions…</Text>
        </Box>
        <Text dimColor>
          {retrying ? 'Retrying…' : 'Fetching your CatchCat Code sessions…'}
        </Text>
      </Box>;
  }
  if (loadErrorType) {
    return <Box flexDirection="column" padding={1}>
        <Text bold color="error">
          Error loading CatchCat Code sessions
        </Text>
  ...
  if (sessions.length === 0) {
    return <Box flexDirection="column" padding={1}>
        <Text bold>
          No CatchCat Code sessions found
  ...
  switch (errorType) {
    ...
    case 'other':
      return <Box marginY={1} flexDirection="row">
          <Text dimColor>Sorry, CatchCat Code encountered an error</Text>
        </Box>;
  }
}
