...
Hooks 是用户定义的 shell 命令，在 CatchCat Code 生命周期的特定时刻自动执行。

类比：React 的 `useEffect` 让你在组件渲染后执行自定义逻辑。CatchCat Code 的 Hooks 让你在 AI 的关键行为前后执行自定义脚本。
...
<CardGroup cols={2}>
  ...
  <Card title="修改行为" icon="pen">
    返回结构化的 JSON 输出，影响 CatchCat Code 的后续行为
  </Card>
